...
async def category_create(user:CategoryInputSchema,db: Session = Depends(get_db)):
...
async def delete_category(category_id: int,db: Session = Depends(get_db)):
    category_db = db.query(Category).filter(Category.id == category_id).first()
    if not category_db:
        raise HTTPException( status_code=404,detail='Мындай категори жок')
